    ...
    with placeholder.container():
        ...
        for ticker in selected_tickers:
            try:
                ...
                # 新增：综合解读（最后 5 根 K 线）（最小改动，添加VWAP/MFI/OBV提及）
                def generate_comprehensive_interpretation(data):
                    ...
                    if bullish_count >= 3 and high_volume_count >= 3:
                        return f"最近五日多方主導，出現多根看漲形態（如大陽線或看漲吞噬）且多伴隨放量，市場呈現強勢上漲趨勢，{vwap_trend}，{mfi_level}，{obv_trend}，建議關注買入機會。"
                    elif bearish_count >= 3 and high_volume_count >= 3:
                        return f"最近五日空方主導，出現多根看跌形態（如大陰線或看跌吞噬）且多伴隨放量，市場呈現強勢下跌趨勢，{vwap_trend}，{mfi_level}，{obv_trend}，建議注意賣出風險。"
                    elif neutral_count >= 3:
                        return f"最近五日多空交戰，型態以十字星或普通K線為主，成交量無明顯趨勢，市場處於盤整或方向不明階段，{vwap_trend}，{mfi_level}，{obv_trend}。"
                    elif bullish_count >= 2 and bearish_count >= 2:
                        return f"最近五日多空激烈爭奪，看漲與看跌形態交替出現，成交量變化不一，市場方向不明，建議觀望，{vwap_trend}，{mfi_level}，{obv_trend}。"
                    else:
                        return f"最近五日市場型態與成交量無明顯趨勢，建議持續觀察後續動向，{vwap_trend}，{mfi_level}，{obv_trend}。"
